...
module Plutus.Test.Model.Fork.Ledger.Crypto
    ( module Export
    , PubKey(..)
    , PrivateKey(..)
    , Signature(..)
...
import Plutus.Test.Model.Fork.Data.Aeson.Extras qualified as JSON
...
import Plutus.V1.Ledger.Crypto as Export hiding (Signature, getSignature, PubKey, getPubKey, PrivateKey, getPrivateKey)
...
instance ToHttpApiData LedgerBytes where
    toUrlPiece = JSON.encodeByteString . KB.bytes
...
newtype PubKey = PubKey { getPubKey :: LedgerBytes }
    ...
    deriving newtype (PlutusTx.Eq, PlutusTx.Ord, Serialise, ToJSON, FromJSON, PlutusTx.ToData, PlutusTx.FromData, PlutusTx.UnsafeFromData)
